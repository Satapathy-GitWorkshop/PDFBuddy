...
import {
  ...
  ChevronRight, Home, Clock, Shield, FileText, Upload
} from 'lucide-react'
...
// PreviewToolPage — split layout: preview left, options right
// Accepts same props as ToolPage plus:
//   overlay     — computed overlay descriptor passed down to PDFPreview
//   renderOverlay(canvasRef, options) — called after debounce to draw on overlay canvas
export default function PreviewToolPage({
  ...
  color = '#6366f1',
  accept,
  options,        // JSX options panel (rendered in right column)
  onProcess,
  buttonLabel = 'Process File',
  overlay = null, // passed straight to PDFPreview
}) {
        ...
        <AnimatePresence mode="wait">
          ...
          {!result && (
            <motion.div key="configure"
              ...
              className={`grid gap-6 ${hasFile ? 'grid-cols-1 lg:grid-cols-[1fr_380px]' : 'grid-cols-1 max-w-2xl'}`}
            >
              {/* LEFT — Preview (shown after file selected) or uploader */}
              <div>
                {!hasFile ? (
                  <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8">
                    <FileUploader files={files} setFiles={setFiles} accept={accept} multiple={false} />
                  </div>
                ) : (
                  <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}>
                    <PDFPreview ref={previewRef} file={files[0]} renderWidth={520} overlay={overlay} />
                  </motion.div>
                )}
              </div>

              {/* RIGHT — Options + action (only after file selected) */}
              <AnimatePresence>
                {hasFile && (
                  <motion.div
                  ...
                  >
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={() => setFiles([])}
                          className="text-xs text-slate-400 hover:text-indigo-600 transition flex items-center gap-1 shrink-0 ml-2"
                        >
